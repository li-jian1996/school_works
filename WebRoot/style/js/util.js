...
function gettwoCategory(url){
	...
	$.ajax({
		...
		$.each(data, function(i, n){
			var option=$("<option></option>");
			option.val(n.tcid);
			option.html(n.tcname);
			$("select[id='tcid']").append(option);	
...
function show_tcid(v){
	$("#tcid").empty();
	$.ajax({
		url : "/shop/admin/getTwocategoriesList",
		type : "post",
	    data: {cid:v},
		dataType : "json",//回调
		success : function(data){
		...
		for(var i=0;i<1;i++){
			if(files[i]){
				...
				reader.onload=function(e){
					var img=$("<img  style='width:100px' src='"+e.target.result+"'><img>");
					imgs.append(img);
				};
			}
		}
